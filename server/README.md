...
This covers a fake/instant top-up. For a real charge, see Payments below —
it credits the same `user.wallet.balance` and writes to the same
`Transaction` collection (with `method: "razorpay"`), so both paths feed
the same Wallet/Payment History UI without any extra plumbing.

## Payments API (Razorpay)

Base path: `/api/payments`. Real wallet top-ups via
[Razorpay](https://razorpay.com) — currently INR only, capped at ₹50,000
per top-up. Requires `RAZORPAY_KEY_ID` / `RAZORPAY_KEY_SECRET` in `.env`
(get them from the Razorpay Dashboard → Settings → API Keys; use the
**test** keys until you're ready to go live). The flow is: create an order
server-side → the frontend opens Razorpay's checkout modal with it → on
success, the frontend calls `/verify`.

| Method | Route | Auth | Body | Description |
|---|---|:---:|---|---|
| POST | `/api/payments/razorpay/create-order` | Protected | `{ amount }` (rupees) | Creates a Razorpay order → `{ id, amount, currency, keyId }`. `amount` is capped at 50000 and re-sent in paise; `keyId` is Razorpay's *public* key, safe to hand to the frontend's checkout.js — the secret never leaves the server. |
| POST | `/api/payments/razorpay/verify` | Protected | `{ razorpay_order_id, razorpay_payment_id, razorpay_signature }` | Called right after the checkout modal succeeds client-side. Recomputes the HMAC signature itself (rejects `400` if it doesn't match — this is what stops someone calling this directly and claiming they paid without paying), then fetches the real captured amount from Razorpay and credits the wallet → `{ ok: true, wallet: { balance }, transaction }` |
| POST | `/api/payments/razorpay/webhook` | Signature only, no JWT | Raw Razorpay event | Optional but recommended — the `/verify` call only fires if the user's browser is still open when the JS callback runs; the webhook is what makes a payment durable against a dropped connection. Configure in Razorpay Dashboard → Webhooks, pointed at this URL, subscribed to `payment.captured`, with a **separate** secret in `RAZORPAY_WEBHOOK_SECRET`. |

Both `/verify` and the webhook end up calling the same internal
`creditWalletForPayment()`, keyed on Razorpay's `payment_id` — if both fire
for the same payment (a real possibility: the tab closes right after the
checkout callback but before `/verify`'s request lands, and the webhook
picks it up moments later), the second call is a no-op instead of a
double-credit.

Note the currency mismatch with the rest of the app: `user.wallet.balance`
elsewhere is treated as generic decimal "whole currency units" (the
frontend shows it with a `$`), while Razorpay here is INR-only and the
rupee amount is credited into that same balance 1:1. Fine for a demo/single-
currency deployment; if you need actual multi-currency support, that
conversion needs to happen explicitly before crediting.
...
skillswap-backend/
  ...
  src/
    ...
    models/
      ...
      Transaction.js                Wallet ledger entries (topup/session_payment/refund; topup may be via Razorpay)
    ...
    controllers/
      ...
      paymentsController.js         Razorpay create-order/verify/webhook (real charges, see lib/razorpayClient.js)
    ...
    lib/
      razorpayClient.js            Razorpay SDK instance, reads RAZORPAY_KEY_ID/SECRET
