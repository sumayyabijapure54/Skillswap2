...
// Skill with neither a linked video nor any lessons, so a mentor/admin can
// go fix each one manually. Never invents lessons or a quiz for them.
//
// Usage:
...
async function main() {
  ...
  const offenders = await Skill.find({
    $and: [
      { $or: [{ youtubeVideo: null }, { youtubeVideo: { $exists: false } }] },
  ...
  if (offenders.length === 0) {
    ...
    console.log(`Found ${offenders.length} skill(s) with no lessons and no youtubeVideo:\n`);
    ...
    console.log('Each of these needs a mentor to add a YouTube video or lessons via Edit Course before its quiz will work.');
  }
