...
export default function SignUp(){
  ...
  const onSubmit = async (e)=>{
    ...
    setSubmitting(false);
    if(!res.ok){
      setErrors(e => ({ ...e, form: res.error || 'Something went wrong. Please try again.' }));
      return;
    }
  ...
  return (
    <AuthLayout variant="signup">
      ...
      <form onSubmit={onSubmit} noValidate>
        ...
        <button type="submit" className="btn-primary-lg btn-full" disabled={submitting}>{submitting ? 'Creating account…' : 'Create account →'}</button>
      </form>
    </AuthLayout>
  );
}
