...
export default function ResetPassword(){
  ...
  const onSubmit = async (e)=>{
    e.preventDefault();
    const e2 = {};
    if(!token) e2.form = 'This reset link is missing or invalid — request a new one.';
    ...
    setSubmitting(true);
    const res = await confirmPasswordReset(token, form.password);
    setSubmitting(false);
    if(!res.ok){ setErrors({ form: res.error || 'That reset link is invalid or has expired.' }); return; }
    setDone(true);
  };
      ...
      {!done ? (
        <>
          ...
          <form onSubmit={onSubmit} noValidate>
            ...
            <button type="submit" className="btn-primary-lg btn-full" disabled={submitting}>{submitting ? 'Resetting…' : 'Reset password →'}</button>
          </form>
        </>
      ) : (
